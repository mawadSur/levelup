/**
 * Scenario markdown parser (web / bundle-side copy).
 *
 * Duplicate of `packages/db/src/scenario-parser.ts` — see that file for the
 * dependency-graph reason both copies exist. The two MUST stay in sync.
 */
...
function parseScenes(body: string): ScenarioScene[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (dialogue) {
      ...
      const lineEntry: ScenarioLine = { character, text };
      current.lines.push(lineEntry);
