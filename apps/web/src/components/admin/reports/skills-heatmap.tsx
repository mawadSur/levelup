...
export function SkillsHeatmap({ cells }: SkillsHeatmapProps) {
  if (cells.length === 0) {
    return <p className="text-sm text-muted-foreground italic">No heatmap data available.</p>;
  }
    ...
    <div className="space-y-4">
      <div className="overflow-x-auto">
        <table className="min-w-max text-xs border-collapse">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-background px-3 py-2 text-left font-semibold text-muted-foreground min-w-[140px]">
                Department
              </th>
              {paths.map((p) => (
                <th
                  key={p}
                  className="px-3 py-2 text-center font-semibold text-muted-foreground max-w-[120px]"
                >
                  ...
                  {rowCells.map((cell, pathIdx) => (
                    <td key={pathIdx} className="px-1.5 py-1.5 text-center">
                      {cell ? (
                        ...
                        <div className="rounded bg-muted/40 px-2 py-1.5 text-muted-foreground/40">
                          —
                        </div>
                      )}
                    </td>
          ...
          <tfoot>
            <tr className="border-t-2 border-border bg-muted/30">
              <td className="sticky left-0 z-10 bg-muted/30 px-3 py-2 font-semibold text-foreground">
                Totals
              </td>
              {paths.map((_, pathIdx) => {
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        ...
        {[
          ...
          <span key={label} className="flex items-center gap-1">
            <span className={`inline-block h-3 w-5 rounded ${cls} border border-border`} />
            {label}
          </span>
        ))}
      </div>
