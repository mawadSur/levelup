...
export function PromptSuggestionCard({ text, onPick }: PromptSuggestionCardProps) {
  return (
    <button
      type="button"
      onClick={() => onPick(text)}
      className="group flex w-full items-start gap-3 rounded-lg border bg-background p-4 text-left text-sm shadow-sm transition-all hover:-translate-y-0.5 hover:border-indigo-300 hover:shadow-md dark:hover:border-indigo-700"
    >
      ...
      <span className="flex-1 leading-relaxed text-foreground">{text}</span>
    </button>
  );
}
