...
/** Recursively scrub sensitive keys at any depth. Mutates `node` in place. */
...
function scrubEvent(event: Sentry.ErrorEvent): Sentry.ErrorEvent {
  ...
  if (event.request) {
    // Drop request body for auth/users routes.
    ...
    try {
      ...
      // Malformed URL — be safe and drop the body anyway.
      delete event.request.data;
    }
    // Always strip auth-bearing headers.
