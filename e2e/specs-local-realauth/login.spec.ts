...
test.describe('Deployed sign-in (real Supabase auth)', () => {
  for (const user of DEMO_USERS) {
    test(`${user.email} signs in, /auth/me reports ${user.role}, ${user.landing} loads`, async ({
      ...
      // Step 1 — the form must navigate off /sign-in. Generous timeout for
      // local because Next.js dev compiles /learn (and its many components) on
      // first request — cold compile can take 30s on this app.
      await page
        .waitForURL((url) => !url.pathname.startsWith('/sign-in'), { timeout: 60_000 })
        .catch(async () => {
      ...
      // Locally there's no /api/* Vercel rewrite, so hit the API directly.
      const apiBase = process.env.LOCAL_API_URL ?? 'http://localhost:4100';
      ...
      await page.goto(user.landing, { waitUntil: 'domcontentloaded', timeout: 90_000 });
      const urlObj = new URL(page.url());
      if (urlObj.pathname.startsWith('/sign-in')) {
